...
def detect_from_mask(mask_path, expected_count=0):
    ...
    This replaces Claude Vision's coordinate guessing with exact contour detection.
    """
    ...
    # Collect candidate rectangles
    candidates = []
    # Panels should be at least 1.5% of image area (generous minimum)
    min_area = img_area * 0.015
    # And at most 40% (single panel on a page)
    ...
    for contour in contours:
        ...
        # Aspect ratio filter: panels are roughly 4:3 to 16:9 (0.5 to 3.0 to be safe)
    ...
    # Sort by area descending — panels are the biggest blobs
    candidates.sort(key=lambda r: r['area'], reverse=True)
    
    if expected_count > 0 and len(candidates) > expected_count:
        # Take the top N by area — panels are bigger than text remnants
