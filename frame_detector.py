...
async function detectRectanglesPython(imagePath, expectedCount = null) {
  return new Promise((resolve) => {
    ...
    const tryPython = (cmd) => {
      // Build args: script, imagePath, 'crop', [expectedCount]
      const args = [script, imagePath, 'crop'];
      if (expectedCount) {
        args.push(expectedCount.toString());
      }
      ...
      proc.on('close', code => {
        ...
        try {
          ...
          console.log(`[Storyboard] Found ${result.count} rectangles${expectedCount ? ` (expected ${expectedCount})` : ''}`);
          resolve(result);
        } catch (e) {
...
/**
 * Extract frames using a fixed grid layout (bypasses rectangle detection)
 * Divides the image into cols x rows cells and extracts non-empty ones
 */
async function extractGridFrames(imagePath, cols, rows) {
  const metadata = await sharp(imagePath).metadata();
  const { width, height } = metadata;
  
  // Calculate cell dimensions with some margin for borders/gutters
  const marginPercent = 0.02; // 2% margin on each side
  ...
  // Extract cells in reading order (top to bottom, left to right)
  for (let row = 0; row < rows; row++) {
    for (let col = 0; col < cols; col++) {
      ...
      const cropX = x + inset;
      const cropY = y + inset;
      const cropW = cellWidth - (inset * 2);
      const cropH = cellHeight - (inset * 2);
      ...
      try {
        ...
        // Check if cell is mostly empty (white/light)
        const stats = await sharp(cellBuffer).stats();
        const avgBrightness = (stats.channels[0].mean + stats.channels[1].mean + stats.channels[2].mean) / 3;
        
        // Skip cells that are >95% white (likely empty)
        ...
        images.push(cellBuffer.toString('base64'));
      } catch (e) {
        console.error(`[Storyboard] Grid cell extraction error at ${row},${col}:`, e.message);
      }
    }
  }
  
  console.log(`[Storyboard] Grid ${cols}x${rows}: extracted ${images.length} non-empty frames`);
...
app.post('/api/extract-storyboard', upload.single('pdf'), async (req, res) => {
  ...
  try {
    ...
    // Parse grid parameters if provided (e.g., 3x3)
    const gridCols = parseInt(req.body.gridCols) || 0;
    const gridRows = parseInt(req.body.gridRows) || 0;
    const useGrid = gridCols > 0 && gridRows > 0;
    
    console.log('[Storyboard] Processing:', req.file.originalname, useGrid ? `(grid: ${gridCols}x${gridRows})` : '(auto-detect)');
    const startTime = Date.now();
    ...
    const pageResults = await Promise.all(pageImages.map(async (imagePath, i) => {
      ...
      // Use grid extraction if specified, otherwise auto-detect rectangles
      const [detected, textData] = await Promise.all([
        useGrid ? extractGridFrames(imagePath, gridCols, gridRows) : detectRectangles(imagePath),
